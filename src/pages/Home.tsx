  ...
  return (
    <PageTransition>
      <div className="min-h-screen" style={{ background: 'var(--bg-primary)', color: 'var(--text-primary)' }}>
        ...
        {/* Hero */}
        <section className="relative flex flex-col items-center justify-center text-center px-5 pt-28 sm:pt-48 pb-24 sm:pb-48 gap-6 sm:gap-8 overflow-hidden">
...
{' '}
        ...
        <section className="relative max-w-5xl mx-auto px-4 sm:px-8 pb-20 sm:pb-32 grid grid-cols-1 sm:grid-cols-2 gap-6">
          {features.map(({ icon, title, desc }, i) => (
            <ScrollRevealCard key={title} delay={i * 0.1}>
              <div
                className={`glow-card hover-scale rounded-2xl p-5 sm:p-8 flex flex-col gap-4`}
        ...
        {/* Footer */}
        <footer style={{ position: 'relative', overflow: 'hidden', background: 'var(--bg-primary)', marginTop: '24px' }} aria-label="Site footer">
          <style>{`
            ...
            .footer-social a { transition: background 0.25s, transform 0.25s, box-shadow 0.25s; }
            .footer-social a:active { transform: scale(0.88) !important; }
            .footer-social a:hover svg { animation: slide-in-top 0.25s both; }
            .footer-social .s-ig:hover { background: linear-gradient(135deg,#f09433,#e6683c,#dc2743,#cc2366,#bc1888) !important; transform: scale(1.18); box-shadow: 0 6px 20px rgba(220,39,67,0.45) !important; }
            .footer-social .s-tw:hover { background: #000 !important; transform: scale(1.18); box-shadow: 0 6px 20px rgba(0,0,0,0.5) !important; }
            .footer-social .s-li:hover { background: #0072b1 !important; transform: scale(1.18); box-shadow: 0 6px 20px rgba(0,114,177,0.45) !important; }
            .footer-social .s-wa:hover { background: #25d366 !important; transform: scale(1.18); box-shadow: 0 6px 20px rgba(37,211,102,0.45) !important; }
            .footer-ql a { position: relative; }
            .footer-ql a::before { content: '→'; position: absolute; left: -18px; opacity: 0; transition: opacity 0.2s, left 0.2s; }
            .footer-ql a:hover { color: var(--text-primary) !important; padding-left: 20px !important; }
            .footer-ql a:hover::before { opacity: 1; left: 0; }
            .footer-brand-name {
          ...
          {/* Top gradient border */}
          <div style={{ height: '2px', background: 'linear-gradient(90deg, transparent 0%, #7c3aed 30%, #a855f7 50%, #7c3aed 70%, transparent 100%)' }} />

          {/* Ambient glow */}
          <div style={{ position: 'absolute', top: 0, left: '50%', transform: 'translateX(-50%)', width: '800px', height: '300px', background: 'radial-gradient(ellipse at top, rgba(124,58,237,0.12) 0%, transparent 70%)', pointerEvents: 'none', animation: 'footer-glow-pulse 4s ease-in-out infinite' }} />

          {/* Main grid */}
          <div style={{ position: 'relative', maxWidth: '1100px', margin: '0 auto', padding: '56px 24px 40px', display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(min(220px, 100%), 1fr))', gap: '48px', alignItems: 'start' }}>

            {/* Left — brand + tagline + social */}
            <div style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
              {/* Logo + name */}
              <div style={{ display: 'flex', alignItems: 'center', gap: '14px' }}>
                <img src="/favicon.ico" alt="AI Islam" style={{ width: '48px', height: '48px', borderRadius: '50%', objectFit: 'cover', boxShadow: '0 0 24px rgba(168,85,247,0.5)' }} />
                <span className="footer-brand-name">AI ISLAM</span>
              </div>
              <p style={{ fontSize: '13px', color: 'var(--text-muted)', lineHeight: 1.8, maxWidth: '280px', margin: 0 }}>
                Authentic Islamic guidance powered by AI — fast, accurate, and multilingual.
              </p>

              {/* Social icons */}
              <div className="footer-social" style={{ display: 'flex', gap: '10px', flexWrap: 'wrap' }}>
                {/* Instagram */}
                <a href="#" aria-label="Instagram" className="s-ig" style={{ width: '42px', height: '42px', background: 'rgba(168,85,247,0.15)', border: '1px solid rgba(168,85,247,0.25)', borderRadius: '12px', display: 'flex', alignItems: 'center', justifyContent: 'center', textDecoration: 'none', flexShrink: 0 }}>
                  <svg viewBox="0 0 16 16" style={{ width: '16px' }}><path fill="#fff" d="M8 0C5.829 0 5.556.01 4.703.048 3.85.088 3.269.222 2.76.42a3.917 3.917 0 0 0-1.417.923A3.927 3.927 0 0 0 .42 2.76C.222 3.268.087 3.85.048 4.7.01 5.555 0 5.827 0 8.001c0 2.172.01 2.444.048 3.297.04.852.174 1.433.372 1.942.205.526.478.972.923 1.417.444.445.89.719 1.416.923.51.198 1.09.333 1.942.372C5.555 15.99 5.827 16 8 16s2.444-.01 3.298-.048c.851-.04 1.434-.174 1.943-.372a3.916 3.916 0 0 0 1.416-.923c.445-.445.718-.891.923-1.417.197-.509.332-1.09.372-1.942C15.99 10.445 16 10.173 16 8s-.01-2.445-.048-3.299c-.04-.851-.175-1.433-.372-1.941a3.926 3.926 0 0 0-.923-1.417A3.911 3.911 0 0 0 13.24.42c-.51-.198-1.092-.333-1.943-.372C10.443.01 10.172 0 7.998 0h.003zm-.717 1.442h.718c2.136 0 2.389.007 3.232.046.78.035 1.204.166 1.486.275.373.145.64.319.92.599.28.28.453.546.598.92.11.281.24.705.275 1.485.039.843.047 1.096.047 3.231s-.008 2.389-.047 3.232c-.035.78-.166 1.203-.275 1.485a2.47 2.47 0 0 1-.599.919c-.28.28-.546.453-.92.598-.28.11-.704.24-1.485.276-.843.038-1.096.047-3.232.047s-2.39-.009-3.233-.047c-.78-.036-1.203-.166-1.485-.276a2.478 2.478 0 0 1-.92-.598 2.48 2.48 0 0 1-.6-.92c-.109-.281-.24-.705-.275-1.485-.038-.843-.046-1.096-.046-3.233 0-2.136.008-2.388.046-3.231.036-.78.166-1.204.276-1.486.145-.373.319-.64.599-.92.28-.28.546-.453.92-.598.282-.11.705-.24 1.485-.276.738-.034 1.024-.044 2.515-.045v.002zm4.988 1.328a.96.96 0 1 0 0 1.92.96.96 0 0 0 0-1.92zm-4.27 1.122a4.109 4.109 0 1 0 0 8.217 4.109 4.109 0 0 0 0-8.217zm0 1.441a2.667 2.667 0 1 1 0 5.334 2.667 2.667 0 0 1 0-5.334z"/></svg>
                </a>
                {/* Twitter/X */}
                <a href="#" aria-label="Twitter / X" className="s-tw" style={{ width: '42px', height: '42px', background: 'rgba(168,85,247,0.15)', border: '1px solid rgba(168,85,247,0.25)', borderRadius: '12px', display: 'flex', alignItems: 'center', justifyContent: 'center', textDecoration: 'none', flexShrink: 0 }}>
                ...
                {/* WhatsApp */}
                <a href="#" aria-label="WhatsApp" className="s-wa" style={{ width: '42px', height: '42px', background: 'rgba(168,85,247,0.15)', border: '1px solid rgba(168,85,247,0.25)', borderRadius: '12px', display: 'flex', alignItems: 'center', justifyContent: 'center', textDecoration: 'none', flexShrink: 0 }}>
                  <svg viewBox="0 0 16 16" style={{ width: '16px' }}><path fill="#fff" d="M13.601 2.326A7.854 7.854 0 0 0 7.994 0C3.627 0 .068 3.558.064 7.926c0 1.399.366 2.76 1.057 3.965L0 16l4.204-1.102a7.933 7.933 0 0 0 3.79.965h.004c4.368 0 7.926-3.558 7.93-7.93A7.898 7.898 0 0 0 13.6 2.326zM7.994 14.521a6.573 6.573 0 0 1-3.356-.92l-.24-.144-2.494.654.666-2.433-.156-.251a6.56 6.56 0 0 1-1.007-3.505c0-3.626 2.957-6.584 6.591-6.584a6.56 6.56 0 0 1 4.66 1.931 6.557 6.557 0 0 1 1.928 4.66c-.004 3.639-2.961 6.592-6.592 6.592zm3.615-4.934c-.197-.099-1.17-.578-1.353-.646-.182-.065-.315-.099-.445.099-.133.197-.513.646-.627.775-.114.133-.232.148-.43.05-.197-.1-.836-.308-1.592-.985-.59-.525-.985-1.175-1.103-1.372-.114-.198-.011-.304.088-.403.087-.088.197-.232.296-.346.1-.114.133-.198.198-.33.065-.134.034-.248-.015-.347-.05-.099-.445-1.076-.612-1.47-.16-.389-.323-.335-.445-.34-.114-.007-.247-.007-.38-.007a.729.729 0 0 0-.529.247c-.182.198-.691.677-.691 1.654 0 .977.71 1.916.81 2.049.098.133 1.394 2.132 3.383 2.992.47.205.84.326 1.129.418.475.152.904.129 1.246.08.38-.058 1.171-.48 1.338-.943.164-.464.164-.86.114-.943-.049-.084-.182-.133-.38-.232z"/></svg>
                </a>
              </div>
            </div>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
              <p style={{ fontSize: '10px', fontWeight: 800, letterSpacing: '0.15em', textTransform: 'uppercase', color: 'var(--accent)', margin: 0 }}>Navigation</p>
              <div className="footer-ql" style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
                {[
                  { label: 'Home', href: '/' },
                  ...
                  <a key={label} href={href} style={{ fontSize: '14px', color: 'var(--text-secondary)', textDecoration: 'none', transition: 'color 0.2s, padding-left 0.2s', paddingLeft: '0', display: 'block' }}>
                    {label}
                  </a>
                ))}
              </div>
            ...
            {/* Right — mission card */}
            <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
              <p style={{ fontSize: '10px', fontWeight: 800, letterSpacing: '0.15em', textTransform: 'uppercase', color: 'var(--accent)', margin: 0 }}>Our Mission</p>
              <div style={{ padding: '20px', borderRadius: '16px', background: 'rgba(124,58,237,0.08)', border: '1px solid rgba(124,58,237,0.2)' }}>
                <p style={{ fontSize: '13px', color: 'var(--text-secondary)', lineHeight: 1.8, margin: 0 }}>
                  Bridging the gap between technology and Islamic knowledge — making authentic guidance accessible to every Muslim, everywhere.
                </p>
              </div>
            </div>
          </div>
          ...
          {/* Divider */}
          <div style={{ height: '1px', background: 'linear-gradient(90deg, transparent, rgba(124,58,237,0.4), transparent)', margin: '0 24px' }} />

          {/* Bottom bar */}
          <div style={{ maxWidth: '1100px', margin: '0 auto', padding: '20px 24px', display: 'flex', flexWrap: 'wrap', alignItems: 'center', justifyContent: 'space-between', gap: '10px' }}>
